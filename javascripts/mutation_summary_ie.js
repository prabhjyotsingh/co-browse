...
//from http://code.google.com/p/mutation-summary/source/browse/src/mutation-summary.js
...
var NodeMap = (function() {
    ...
    NodeMap.prototype.nodeId = function(node) {
        ...
        if(!id) {
            id = parseInt(NodeMap['nextId_']) + 1;
            NodeMap['nextId_'] = id;
            try {
                node[NodeMap['ID_PROP']] = id;
            } catch(e) {
//                console.log("Error::mutation_summary_ie::NodeMap.prototype.nodeId::");
//                                console.log(e);
            }
    ...
    NodeMap.prototype.del = function(node) {
        var id = this.nodeId(node);
...
var NodeChange = (function() {
    function NodeChange(node, childList, attributes, characterData, oldParentNode, added, attributeOldValues,
        ...
        this.isCaseInsensitive =
            this.node.nodeType === Node.ELEMENT_NODE && this.node instanceof HTMLElement && this.node.ownerDocument instanceof HTMLDocument;
    }
...
var MutationProjection = (function() {
        ...
        for(var i = 0 ; i < this.mutations.length ; i++) {
            ...
            for(var j = 0 ; j < mutation.removedNodes.length ; j++) {
                ...
                if(change.added.has(node)) {
                    change.added.del(node);
                } else {
                    change.removed.set(node, true);
                    change.maybeMoved.del(node);
                }
    ...
    MutationProjection.prototype.wasReordered = function(node) {
        ...
        JQ.each(change.maybeMoved.keys(), isMoved);
        return change.moved.get(node);
    };
        ...
        if(query.all || query.attribute || query.attributeList) {
            ...
            if(query.attribute) {
                ...
                if(query.attributeList) {
                    JQ.each(query.attributeList, function(attrName) {
...
var Selector = (function() {
    ...
    function caseInsensitiveTagName(tagName) {
        return tagName.toUpperCase();
    }

    function selectorString(tagName, qualifiers) {
        return tagName + qualifiers.join('');
    }

    Selector.prototype.isMatching = function(el) {
        return el[Selector.matchesSelector](selectorString(this.tagName, this.qualifiers));
    };
    ...
    Selector.prototype.wasMatching = function(el, change, isMatching) {
        ...
        var tagName = change.isCaseInsensitive ? caseInsensitiveTagName(this.tagName) : this.tagName;
...
function elementFilterAttributes(selectors) {
    ...
    JQ.each(selectors, function(selector) {
        JQ.each(selector.qualifiers, function(qualifier) {
...
var MutationSummary = (function() {
    ...
    MutationSummary.createObserverOptions = function(queries) {
        ...
        function observeAttributes(attributes) {
            ...
            JQ.each(attributes, function(attribute) {
                attributeFilter[attribute] = true;
                attributeFilter[attribute.toLowerCase()] = true;
            });
        }

        JQ.each(queries, function(query) {
            if(query.characterData) {
        ...
        if(attributeFilter)
            observerOptions.attributeFilter = getKeys(attributeFilter);
    ...
    MutationSummary.validateOptions = function(options) {
        ...
        for(var i = 0 ; i < options.queries.length ; i++) {
            ...
            if('element' in request) {
                var requestOptionCount = getKeys(request).length;
                var query = {
    ...
    MutationSummary.prototype.createSummaries = function(mutations) {
        ...
        var projection = new MutationProjection(this.root, mutations, this.elementFilter, this.calcReordered,
    ...
    MutationSummary.prototype.checkpointQueryValidators = function() {
        JQ.each(this.queryValidators, function(validator) {
            if(validator)
    ...
    MutationSummary.prototype.runQueryValidators = function(summaries) {
        JQ.each(this.queryValidators, function(validator, index) {
            if(validator)
    ...
    MutationSummary.prototype.changesToReport = function(summaries) {
        return summaries.some(function(summary) {
            ...
            if(summary.attributeChanged) {
                var attrNames = getKeys(summary.attributeChanged);
